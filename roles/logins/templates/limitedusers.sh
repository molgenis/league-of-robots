...
_memlimit="{{ (ansible_facts.memory_mb.real.total / 25) | round | int }}" # memory > 100GB, each user gets 4% of total memory
{% elif ansible_facts.memory_mb.real.total >= 2000 %} 
_memlimit="{{ (ansible_facts.memory_mb.real.total / 10) | round | int }}" # memory > 2G and memory < 100G, each user gets 10% of all memory
{% else %}
_memlimit="{{ (ansible_facts.memory_mb.real.total / 3) | round | int }}" # memory < 2G, each user gets 33% of all memory
{% endif %}
